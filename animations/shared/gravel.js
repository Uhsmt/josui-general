...
class GravelAnimation {
  ...
  createInitialTravelingParticles(area) {
    ...
    // area2は半分に減らす
    const count =
      area.id === 'gravel-area-2'
        ? this.config.initialTravelParticles / 2
        : this.config.initialTravelParticles;
  ...
  createInitialParticles(area) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 砂利の色（茶色系のランダムな色）
      const colors = ['#8b5a2b', '#654321', '#a0522d', '#785032'];
      const color = colors[Math.floor(Math.random() * colors.length)];
