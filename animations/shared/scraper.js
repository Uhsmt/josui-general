...
class ShochinAnimation {
  ...
  initAxles() {
    ...
    const axlePositions = [
      ...
      {
        x: this.track.bottomRight.x - offset * 2 - inset,
        y: this.track.bottomRight.y - offset * 1.5 - inset,
      }, // 右下
  ...
  getBladeTransform(position) {
    ...
    // トラックの太さの半分だけ左上に移動
    x =
      this.track.bottomLeft.x +
      leftDx * ratio -
      this.CONFIG.bladeSize -
      this.CONFIG.borderWidth / 2;
    y =
      this.track.bottomLeft.y +
      leftDy * ratio -
      this.CONFIG.bladeSize / 2 -
      this.CONFIG.borderWidth / 2;
    direction = 'diagonal';
  ...
  updateBlades(deltaFactor = 1) {
    this.blades.forEach((blade) => {
      ...
      if (this.reverseDirection) {
        blade.position =
          (blade.position - this.CONFIG.speed * deltaFactor + this.perimeter) % this.perimeter;
      } else {
